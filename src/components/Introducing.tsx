  ...
  return (
    <section className="px-6 py-20 md:py-28" style={{ borderTop: `1px solid ${BORDER}` }}>
      <div className="max-w-5xl mx-auto text-center">
        <p className="text-[11px] font-semibold mb-4" style={{ color: ORANGE, letterSpacing: "0.28em" }}>
          INTRODUCING
        </p>
        ...
        <p className="max-w-2xl mx-auto text-[15px] leading-[1.75]" style={{ color: "#C8C3B7" }}>
          The first community in India built to make a bootstrapped business owner{" "}
          <em>undeniable</em> — without funding, without an MBA, and without figuring it out
          alone.
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-5 mt-12 text-left">
          {[
            ...
            <div
              ...
              style={{ background: SURFACE, borderTop: `3px solid ${ORANGE}`, border: `1px solid ${BORDER}`, borderTopColor: ORANGE, borderTopWidth: 3 }}
            >
              <p className="text-[11px] font-semibold mb-4" style={{ color: ORANGE, letterSpacing: "0.2em" }}>
                {c.h}
              </p>
              <h3 className="font-display text-2xl text-white mb-3">{c.t}</h3>
              <p className="text-[14px] leading-[1.7]" style={{ color: "#B8B3A7" }}>{c.b}</p>
            </div>
          ))}
        </div>
        ...
        <p className="mt-3 text-[14px] max-w-2xl mx-auto" style={{ color: MUTED }}>
          One honest thing, because I don't hide the ball: you'll want a paid AI tool —
          Claude or ChatGPT — to build alongside me. A few hundred rupees a month, and it's
          the one thing you bring. Everything else is in here.
        </p>

        <div className="grid md:grid-cols-2 gap-10 mt-16 text-left">
          <div>
            <h4 className="text-white font-bold text-lg mb-5">And it works even if…</h4>
            <ul className="space-y-3">
              {works.map((w) => (
                ...
                >
                  ...
                  <span className="text-[14px]" style={{ color: "#C8C3B7" }}>{w}</span>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className="text-white font-bold text-lg mb-5">What it isn't</h4>
            <ul className="space-y-3">
              {isnt.map((w) => (
                <li
                  key={w}
                  ...
                  style={{ background: SURFACE, border: `1px solid ${BORDER}` }}
                >
                  <X className="w-4 h-4 mt-1 flex-shrink-0" style={{ color: "#E54848" }} />
                  <span className="text-[14px]" style={{ color: "#C8C3B7" }}>{w}</span>
                </li>
              ))}
            </ul>
            <p className="mt-5 text-[14px] leading-[1.7]" style={{ color: MUTED }}>
              This is for the owner willing to do real work, one month at a time, to build
              something AI can't wipe out. If that's you, you're in the right place.
            </p>
          </div>
        </div>
      </div>
